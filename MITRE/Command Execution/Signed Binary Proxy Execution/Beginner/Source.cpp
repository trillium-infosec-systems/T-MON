...
bool _commandMatch(const std::string& s, char delimiter)
{
	...
	if (strcmp(tokens[0].c_str(), "bitsadmin") == 0)
	{
		return true;
	}
	else if (strcmp(tokens[0].c_str(), "msiexec.exe") == 0 || strcmp(tokens[0].c_str(), "msiexec") == 0)
	{
		if (tokens[3].find("http://") == std::string::npos)
		{
			return true;
		}
		else
...
int main()
{
	...
	char *buff = ""
		...
		"1:\tBitsadmin can be used to Download File\n"
		"2:\tExecute MSI Installer via msiexec\n";
