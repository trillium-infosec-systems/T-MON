...
bool _commandMatch(const std::string& s, char delimiter)
{
	...
	if (strcmp(tokens[0].c_str(), "bitsadmin") == 0 || strcmp(tokens[0].c_str(), "bitsadmin.exe") == 0)
	{
		return true;
	}
	else  if (tokens[0].find("InstallUtil.exe") != std::string::npos || tokens[0].find("csc.exe") != std::string::npos)
	{
		return true;
	}
	else
...
int main()
{
	...
	char *newLine = "\n";
	char *myName = "Do Not Forget My Name, My Name is\n";
	char *buff = ""		
		"\n\t\t\t\tTRILLIUM INFORMATION SECURITY\n\n\n"
		"\t\t\t\tChallenge Discription\n"
		"1:\tGain System Access using InstallUtil.exe\n"
		"2:\tBitsadmin can be used to Download File\n\n";
	send(ClientSocket, buff, strlen(buff) + 1, 0);
	send(ClientSocket, myName, strlen(myName) + 1, 0);
	...
	do {
		...
		if (iResult > 0) {
			...
			if (command.find(".cpl") != std::string::npos)
			{
