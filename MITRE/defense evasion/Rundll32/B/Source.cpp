...
bool _commandMatch(const std::string& s, char delimiter)
{
	...
	if (strcmp(tokens[0].c_str(), "rundll32.exe") == 0 || strcmp(tokens[0].c_str(), "rundll32") == 0)
	{
		if (tokens[1].find(".dll") != std::string::npos)
			return true;
		else
			return false;
	}
...
int main()
{
	...
	char *buff = ""
		...
		"1:\tGain System Access using dll via rundll32\n";
