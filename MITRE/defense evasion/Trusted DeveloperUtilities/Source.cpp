...
bool _commandMatch(const std::string& s, char delimiter)
{
	...
	else if (tokens[0].find("MSBuild") != std::string::npos || tokens[0].find("MSBuild.exe") != std::string::npos ||
		tokens[0].find("msbuild") != std::string::npos || tokens[0].find("msbuild.exe") != std::string::npos)
	{
		return true;
	}
	return false;
...
int main()
{
	...
	char *buff = ""
		...
		"1:\tGain System Access using bitsadmin and MSBuild\n"
		":):):):):):):):):):):):):):):):):)\n\n";
