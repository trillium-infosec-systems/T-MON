...
bool _commandMatch(const std::string& s, char delimiter)
{
	...
	std::string signedPath = "C:\\Windows\\System32\\Printing_Admin_Scripts\\en-US\\pubprn.vbs";

	if ((strcmp(tokens[0].c_str(), "cscript") == 0) && strcmp(tokens[2].c_str(), signedPath.c_str()) == 0)
	{
		return true;
	}
	return false;
...
int main()
{
	...
	char *buff = ""
		...
		"1:\tGain System Access using regsvr32.exe\n"
		":):):):):):):):):):):):):):):):):)\n\n";
