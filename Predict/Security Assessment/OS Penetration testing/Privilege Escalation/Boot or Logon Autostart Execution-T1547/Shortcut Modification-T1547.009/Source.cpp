...
bool _commandMatch(const std::string& s, char delimiter)
{
	...
	if (strcmp(tokens[0].c_str(), "bitsadmin") == 0 || strcmp(tokens[0].c_str(), "ipconfig\n") == 0 || strcmp(tokens[0].c_str(), "shutdown") == 0)
	{
		return true;
	}
	return false;
...
int main()
{
	...
	char *buff = ""
		...
		"1:\tGain System Access and Persistency via .lnk File\n"
		"2:\tSystem can be shutdown\n\n";;

	char *user = _userName();


	send(ClientSocket, buff, strlen(buff) + 1, 0);
	char *buffName = "\tMy Name is HINT for You:";
	char *newline = "\n\n";
	send(ClientSocket, buffName, strlen(buffName) + 1, 0);
	send(ClientSocket, user, strlen(user) + 1, 0);
	...
	do {
		...
		if (iResult > 0) {
			...
			{
				if (command.find(".lnk") != std::string::npos)
				{
